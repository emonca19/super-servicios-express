...
const color = body('color').optional({values:'falsy'}).isString().isLength({max:30});
const placas = body('placas').isString().isLength({min:3, max:30});
const numero_serie = body('numero_serie').optional({values:'falsy'}).isString().isLength({max:50});
...
exports.updateRules = [param('id').isInt(), marca.optional({values:'falsy'}), modelo.optional({values:'falsy'}),
  anio.optional({values:'falsy'}), color, placas.optional({values:'falsy'}),
  numero_serie, id_cliente.optional({values:'falsy'})];
